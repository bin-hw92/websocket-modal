...
const Answerer = () => {
  ...
  const createAnswer = () => {
    ...
    peerRef.current.on('data', (event) => {
      console.log('peer Data', event);
    });
    peerRef.current.on('message', (event) => {
  ...
  const sendMessage  = (text) => {
    if (dataChannelRef.current && dataChannelRef.current.readyState === 'open') {
      ...
      socketRef.current.emit('sendMessage', 'user-1', 'message2222 Text'); //상대방 ID 등록
      dataChannelRef.current.send('testtes22222222');
      peerRef.current.send('testtes2222@@@@@@@@@@@');
  ...
  useEffect(() => {
    socketRef.current = io('https://h2o-e-reg-socket-server.h2osvc.biz/');
    ...
    socketRef.current.on('receiveMessage', (userId, message) => {
